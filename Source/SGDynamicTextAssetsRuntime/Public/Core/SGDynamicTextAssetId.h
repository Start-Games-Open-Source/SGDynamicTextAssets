...
#include "SGDynamicTextAssetId.generated.h"

/**
 * Unique identity wrapper for dynamic text assets.
 *
 * Wraps a raw FGuid in a dedicated type so that dynamic text asset identity is
 * distinguishable at the type level from arbitrary GUIDs. This enables
 * future identity system changes (namespaces, hashing, alternate ID
 * schemes) without modifying every call site.
 *
 * Supports binary serialization, network serialization, and text
 * export/import for clipboard and config round-tripping.
 */
USTRUCT(BlueprintType)
struct SGDYNAMICTEXTASSETSRUNTIME_API FSGDynamicTextAssetId
{
	GENERATED_BODY()
public:

	FSGDynamicTextAssetId() = default;

	FSGDynamicTextAssetId(const FGuid& InGuid);

	explicit FSGDynamicTextAssetId(FGuid&& InGuid);
	...
	/**
	 * Converts a string to a GUID and stores it(if one GUID is already set then it will be overriden).
	...
	/** Generates a brand-new unique dynamic text asset ID. */
	void GenerateNewId();

	/** Generates a brand-new unique dynamic text asset ID. */
	static FSGDynamicTextAssetId NewGeneratedId();

	/** Constructs from an existing FGuid. */
	static FSGDynamicTextAssetId FromGuid(const FGuid& InGuid);

	/**
	 * Parses a dynamic text asset ID from a string.
	 * Returns an invalid ID if parsing fails.
	 *
	 * @param InString The string to parse (hyphenated GUID format)
	 * @return The parsed FSGDynamicTextAssetId, or an invalid ID on failure
	 */
	static FSGDynamicTextAssetId FromString(const FString& InString);
	...
	friend uint32 GetTypeHash(const FSGDynamicTextAssetId& Id)
	{
	 ...
	 */
	bool ExportTextItem(FString& ValueStr, const FSGDynamicTextAssetId& DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const;
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FSGDynamicTextAssetIdGuidChangeSignature, FGuid/*Previous*/, FGuid/*New*/);
	/**
	 * For editor tooling, broadcasts when this ID's GUID is changed.
	 * Param 1 = Previous GUID
	 * Param 2 = New/Current GUID
	 */
	FSGDynamicTextAssetIdGuidChangeSignature OnGuidChanged_Editor;
...
template<>
struct TStructOpsTypeTraits<FSGDynamicTextAssetId> : public TStructOpsTypeTraitsBase2<FSGDynamicTextAssetId>
{
	enum
	{
		WithSerializer = true,
